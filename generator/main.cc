...
int main(int argc, char**argv)
{
    ...
    headerfilename = std::string(argv[1]) + ".h";
    implfilename = std::string(argv[1]) + ".c";
    if( !option::outdir.empty() )
    {
        headerfilename = (option::outdir / path(std::string(argv[1]) + ".h")).string();
        implfilename = (option::outdir / path(std::string(argv[1]) + ".c")).string();
    }
    ...
    try {
        ...
        std::cout << "processing: " << argv[1] << " into: " << headerfilename << ", "
                  << implfilename << '\n';
